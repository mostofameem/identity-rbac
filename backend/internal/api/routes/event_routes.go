...
func (server *Server) initEventRoutes(mux *http.ServeMux, manager *middlewares.Manager) {
	...
	mux.Handle(
		...
		manager.With(
			http.HandlerFunc(server.handlers.UpdateEvent),
	...
	mux.Handle(
		...
		manager.With(
			http.HandlerFunc(server.handlers.GetEventParticipants),
			server.middleware.Authorization(middlewares.PARTISIPENT_VIEW_ACCESS),
			server.middleware.AuthenticateJWT,
		),
	)
	...
	mux.Handle(
		...
		manager.With(
			http.HandlerFunc(server.handlers.UpdateEventTypeSettings),
			server.middleware.Authorization(middlewares.EVENT_TYPE_UPDATE_ACCESS),
			server.middleware.AuthenticateJWT,
		),
	)
